...
async function finalSeed() {
  ...
  try {
    ...
    if (users.rows.length === 0) throw new Error('No user found');
    if (media.rows.length === 0) throw new Error('No media found');
    
    const authorId = users.rows[0].id;
    const img1 = media.rows[0].id;
    const img2 = media.rows[1]?.id || img1;

    // 2. Ensure Category (Safe way)
    ...
    const catId = newCat.rows[0].id;
    console.log('Category created:', catId);

    // 3. Clear existing posts to avoid slug conflicts
    ...
    console.log('SUCCESS: Seeded 2 posts.');
