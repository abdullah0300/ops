 ...
 *   - Full descriptions (cleaned from HTML)
 *   - Product specs (dimension, quantity, paper stock, printing, finishing, turnaround)
 *   - Size-based quantity pricing (where available)
 *   - Gallery images (downloaded, converted webp→jpg, uploaded to Payload)
 * 
...
const MYLAR_PRODUCTS = [
  {
    ...
    turnaround: '8 - 12 Business Days, RUSH',
    size_pricing: [
  ...
  },
  {
    ...
    gallery: ['1746602492_custom-lemon-cherry-gelato-bags.webp', '1746602492_hemp-seed-mylar-bags.webp', '1746602492_custom-resealable -mylar-bags.webp', '1746602492_coffee-myalr-bags.webp'],
  ...
  },
  {
    ...
    gallery: [],
    description: 'Our gusset mylar bags feature expandable side or bottom gussets that allow the bag to hold more volume while maintaining a flat profile for easy storage and display. The gusset design enables the bag to stand upright when filled, making it ideal for retail display. Perfect for bulk products, coffee, protein powder, pet treats, and any product that benefits from the additional capacity.',
    dimension: 'All Custom Sizes & Shapes',
    quantity: '250 - 500,000',
    paper_stock: '10pt to 28pt (60lb to 400lb) Eco-Friendly Kraft, E-flute Corrugated, Bux Board, Cardstock',
    printing: 'No Printing, CMYK, CMYK + 1 PMS color, CMYK + 2 PMS colors',
...
async function downloadAndUploadImage(
  ...
  try {
    ...
    // Convert webp -> jpg using sharp (if available) or skip
    try {
      const sharp = require('sharp')
      await sharp(webpPath).jpeg({ quality: 90 }).toFile(jpgPath)
    } catch {
      // If sharp not available, use the webp directly
      fs.copyFileSync(webpPath, jpgPath)
    }
...
async function main() {
  ...
  if (existingCat.docs.length > 0) {
    categoryId = existingCat.docs[0].id as string
  ...
  for (const p of MYLAR_PRODUCTS) {
    ...
    const specifications = [
      ...
      { label: 'Quantity', value: p.quantity || '250 - 500,000' },
      ...
      { label: 'Turnaround', value: p.turnaround || '8 - 12 Business Days' },
    ]
    ...
    if (p.size_pricing && p.size_pricing.length > 0) {
      const firstSize = p.size_pricing[0]
